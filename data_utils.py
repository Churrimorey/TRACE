...
class TemplateSample(BaseModel):
    ...
    def evaluate(self, model_name: str, dataset: str, **kwargs):
        ...
        if dataset == "agentharm":
            self.messages, self.tool_call_chains, self.score, self.refusal = evaluate_agentharm(self.datum, model_name=model_name, field="jailbroken_prompt")
        elif dataset == "advcua":
            self.messages, self.tool_call_chains, self.score, self.refusal = evaluate_advcua(self.datum, model_name=model_name, field="jailbroken_prompt", last_subtask=kwargs.get("last_subtask", False))
        elif dataset == "practical":
            self.messages, self.tool_call_chains, self.score, self.refusal = evaluate_practical(self.datum, model_name=model_name, field="jailbroken_prompt")
